...
public class GlassElevator extends Elevator{
    ...
    public GlassElevator(int elevatorID, String elevatorType, int maxCapacities, double elevatorRequestPercentage, int _currentFloor) {
        super(elevatorID, elevatorType, maxCapacities, elevatorRequestPercentage);
    ...
    public boolean move(direction _direction) {
        if(_direction == direction.UP){
            currentFloor++;
            System.out.println("GlassElevator, ElevatorID: " + elevatorID + " is going up to " + currentFloor);
        }
        else if(_direction == direction.DOWN)
        {
            if (currentFloor != 1){
                currentFloor--;
                System.out.println("GlassElevator, ElevatorID: " + elevatorID + " is going down to " + currentFloor);
            }
            else
                System.out.println("Incorrect Input");
        }
        else  if(_direction == direction.IDLE){
            System.out.println("GlassElevator, ElevatorID: " + elevatorID + " is idle on " + currentFloor);
        }
        else{
            System.out.println("Incorrect Input");
            return false;
